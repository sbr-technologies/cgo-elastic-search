...
class ResumesController < ApplicationController
  ...
  VALID_UPLOAD_FORMATS = [".docx", ".doc", ".pdf", ".html", ".txt"]
...
#  end
  ...
  def guest_search
    ...
    unless params[:q].to_s.blank?
      ...
      unless params[:q][:posted_date].to_s.blank?
        how_many = (params[:q][:posted_date] == "one-month" and 1) || (params[:q][:posted_date] == "three-months" and 3) || (params[:q][:posted_date] == "six-months" and 6)
      else
        how_many = nil
      end
      
      unless params[:q][:state].to_s.blank?
        state = params[:q][:state].map(&:downcase)
      else
        state = nil
      end
    ...
    @applicants = Applicant.search(query, options).results
    end
  end
...
#  end
  ...
  def search
    ...
    (flash.now[:error]="You need to provide a job code for this search") and return if(current_user.employer.is_federal_employer? && params[:job_code].to_s.blank?)
    (flash.now[:error]="You need to provide at least one keyword for this search") and return if(params[:q].to_s.blank? || (params[:q][:keyword].to_s.blank? && params[:q][:applicant_last_name].to_s.blank?))
    (flash.now[:error]="You need to provide keyword of minimum 3 characters for this search") and return if(params[:q][:keyword].to_s.strip.length<3)
...
#    @applicants = Applicant.search(query)
#    @applicants = Applicant.search( query: {match:{last_name: params[:q][:applicant_last_name] } }).results
#    response = Applicant.search(query)

#    @applicants = response.records.where(last_name: params[:q][:applicant_last_name]).to_a
#      @applicants = Applicant.search(query: {
#                                              multi_match: {
#                                                  query: query,
#                                                  fields: ['email', 'name', 'attachment']
#                                                }
#                                              }
#                                      ).results
      unless params[:q][:posted_date].to_s.blank?
    ...
    @applicants = Applicant.search(query, options).page(params[:page]).per_page(Constants::PAGE_SIZE).results
#    render :text => @applicants.inspect
  end
