class RegistrationsController < ApplicationController
    ...
    if current_user.is_a?(Applicant)
      if !current_user.registered?(jobfair)
        ...
        if current_user.security_clearance == '3' && (jobfair.security_clearance_required || jobfair.category == 'security_clearance')
          logger.debug("user should not be able to register");
  ...
  def create
    ...
    if current_user.type == "Applicant" && @jobfair.category == 'military_officer' && (!Constants::TYPE_OF_APPLICANT.select {|x| ['O', 'W'].include?(x[:label][0,1])}.collect {|y| y[:id]}.include?(current_user.type_of_applicant))  
      flash[:notice] = "This Job Fair is for officers only"
      redirect_to jobfairs_path
      return
    end
