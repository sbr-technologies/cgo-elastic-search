...
class JobsController < ApplicationController
    ...
    def search
      ...
      employer_name = @employer_name = (params[:employer][:name] rescue "")

      # Display a message and return if there is no keyword
      ((flash.now[:error] = "You must provide at least one keyword or a company name for your search" ) && return) unless (params[:q][:keyword].present? || params[:employer][:name].present?)
      ...
      logger.info("Searching Jobs: q: " + (query || 'nil') )
      logger.info("Options: " + options.inspect )
      @jobs = Job.search(query, options).page(params[:page] || 1).per_page(Constants::PAGE_SIZE).records
    end
