...
class ResumesUploadJob
  ...
  def perform
      ...
      Zip::ZipFile.open(@resumesFilename) { |rzip|
        ...
        hzip.dir.entries('.').sort.each do |hfilename|
            ...
            begin
              ...
              Delayed::Worker.logger.info("Resume base filename: #{rfilename}")
            ...
            begin
              ...
              if applicant.nil?
                  ...
                  Delayed::Worker.logger.debug("New Applicant !!");
    ...
    Administrator.root_admin.messages << Message.new(:body => "Uploaded resumes for Job Fair: #{@jobfair.name} on #{Date.today}, stats: Total Resumes:[#{@total_records}] Rejected:[#{@total_rejected}]")
    UserMailer.deliver_resumes_upload_complete_notification(@jobfair, @total_records, @total_rejected, @warnings, passw_filename)
  end
